...
function CreatePaletteElements(Id, Name, i) {
    ...
    $('#Palettes_' + i).append("<input type='button' name='EditPalette' value='Edit name' class='EditPalette' id='EditPalette" + Id + "'>");
    $('#Palettes_' + i).append("<input type='button' name='Remove' value='Remove' class='RemovePalette'>");
...
$(document).ready(function () {
    ...
    function GetPalettes(data) {
        ...
        function Success(data) {
            ...
            for (var i = 0; i < data.Palettes.length; i++) {
                CreatePaletteElements(data.Palettes[i].Id, data.Palettes[i].Name, i);
            }
    ...
    $('body').on('click', '.EditPalette', function (event) {
        ...
        if (editPaletteName) {
            ...
            function Success(data) {
                ...
                console.log(data);
                $(event.target).val("Edit name").css({ backgroundColor: "#00baff" });
            ...
            changedPalette = $(event.target);
            var oldPaletteName = $(this).prev().text();
            $(event.target).val("Save").css({ backgroundColor: "rgb(52, 196, 90)" });
    ...
    $(window).click(function (e) {
        var target = $(e.target);
        if (target.is(changedPalette)) return
